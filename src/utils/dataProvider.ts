...
export function authFetch(url: string | URL | Request, options?: RequestInit) {
  return fetch(url, {
    ...options,
    headers: {
      Authorization: "whatever auth happens to be",
...
export type filter<T, K extends object> = T extends object
  ? {
...
export const dataProvider = {
  create: async <TEntity extends object, TResult = string | number>(
    ...
    return authFetch(`${api}/${resource}${queryString ? (resource.includes("?") ? `&${queryString}` : `?${queryString}`) : ""}`, {
      method: "POST",
  ...
  get: async <K extends object, TResult = Result<K>>(resource: string, identifier?: string, queryParams?: filter<TResult, K>) => {
    let queryString: string | undefined
    if (queryParams) {
      const params = new URLSearchParams()
      if (queryParams.like) {
      ...
      Object.keys(queryParams).forEach((e) => {
        if (Array.isArray(queryParams[e as keyof filter<TResult, K>])) {
          /* eslint-disable-next-line @typescript-eslint/no-explicit-any*/
          ;(queryParams[e as keyof filter<TResult, K>] as any[]).forEach((ee) => params.append(e, ee))
        } else {
          /* eslint-disable-next-line @typescript-eslint/no-explicit-any*/
    ...
    return authFetch(`${url}${queryString ? (url.includes("?") ? `&${queryString}` : `?${queryString}`) : ""}`)
      .then((res) => res.json())
      .then((data) => data as Result<TResult>)
      .catch((e) => {
        console.error(e)
  ...
  getList: async <K extends object, TResult extends object = K>(resource: string, queryParams?: listFilter<TResult, K>) => {
    let queryString: string | undefined
    if (queryParams) {
      const params = new URLSearchParams()
      if (queryParams.like) {
      ...
      Object.keys(queryParams).forEach((e) => {
        if (Array.isArray(queryParams[e as keyof listFilter<TResult, K>])) {
          /* eslint-disable-next-line @typescript-eslint/no-explicit-any*/
          ;(queryParams[e as keyof listFilter<TResult, K>] as any[]).forEach((ee) => params.append(e, ee))
        } else {
          params.append(
            e,
            /* eslint-disable-next-line @typescript-eslint/no-explicit-any*/
  ...
  update: async <TEntity extends object, TResult extends object = {}>(
    ...
    return authFetch(`${api}/${postApi}${queryString ? (postApi.includes("?") ? `&${queryString}` : `?${queryString}`) : ""}`, {
      method: "PUT",
      body: JSON.stringify(body),
    })
      .then((res) => res.json())
  ...
  delete: async <TResult>(resource: string, identifier: string, queryParams?: Record<string, baseQueryFilters>) => {
    let queryString: string | undefined
    if (queryParams) {
      const params = new URLSearchParams()
      Object.keys(queryParams).forEach((e) => {
